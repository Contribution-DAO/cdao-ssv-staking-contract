...
import "../interfaces/erc4337/IEntryPointStakeManager.sol";
...
error NotEntryPoint(address _passedAddress);
...
abstract contract ERC4337Account is IAccount, IOwnableWithOperator {
    ...
    function withdrawFromEntryPoint() external {
        if (
            ...
            revert NotAllowedToWithdrawFromEntryPoint();
        }
    ...
    function _validateSignature(
        ...
        if (signer == operator() || signer == client()) {
            validationData = 0;
        } else {
            validationData = 1;
        }
